...
// START/END PRIORITY CONTENT markers and injected into Kiro user messages.
// Default: 0 (disabled). Set to 1 to inject wrapped system prompts.
var systemPromptInjectEnabled atomic.Int32

var promptSectionDelimiterPattern = regexp.MustCompile(`---\s+(?:(?:START|END)\s+)?(?:\w+\s+PROMPT|(?:PRIORITY|NORMAL)(?:\s+CONTENT)?)\s+---`)
